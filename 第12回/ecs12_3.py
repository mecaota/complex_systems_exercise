...
def gpout(label, xlabel, ylabel, name1, name2):
    ...
    str += " using 1:2 with points notitle pointtype 13"
...
if __name__ == "__main__":
    ...
    #ファイル名の設定
    name2 = "{0}{1:.4f}".format("r=", r) + "Hist-sumlog"
    ...
    for i in range(1,hist.size):
        sum = sum + hist[i]
        f1.write(plotout(i, hist[i]))  #Xn-X(n+1)グラフ用プロットデータ
        f2.write(plotout(math.log(0.01*i*0.005), math.log(sum))) #histの合計値をプロット
        #ファイルクローズ
    f1.close()
    f2.close()