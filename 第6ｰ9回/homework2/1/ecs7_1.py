...
if __name__ == "__main__":
    ...
    #ファイル名の設定
    name1 = "{0}{1:.4f}".format("r=", r) + "Logistic"
    #name1 = "{0}{1:.4f}".format("r=", r) + "ReturnMap"

    #ファイルオープン
    f1 = fileopen(name1, ".dat") #n-Xnグラフ用出力ファイル
    gpout("{0}{1:.4f}".format("r = ", r,), "Xn", "X(n+1)", name1)

    #計算
    x = xn = x0 #計算用初期値
    for j in range(n):
        xn = calnext(x, r) #与式兼n+1項の値
        f1.write(plotout(j, xn))  #Xn-X(n+1)グラフ用プロットデータ
