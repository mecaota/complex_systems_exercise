...
def gpout(label, xlabel, ylabel, name1, name2):
    ...
    #str += "set xrange [0.9561:0.9565]" + "\n"
    str += "set xrange [0.9561:0.9565]" + "\n"
    str += "set yrange [0.95605:0.9565]" + "\n"
...
#プロットデータ整形処理
def plotout(x, nx):
    str = "{0:.10f}{1}{2:10f}{3}".format(x, " ", nx, "\n") #データ整形
    return str
...
if __name__ == "__main__":
    ...
    #ファイル名の設定
    name2 = "{0}{1:.4f}".format("r=", r) + "RetLogistic3s"
    name1 = "{0}{1:.4f}".format("r=", r) + "ReturnMap3s"

    #ファイルオープン
    f1 = fileopen(name1, ".dat") #n-Xnグラフ用出力ファイル
    f2 = fileopen(name2, ".dat") #n-Xnグラフ用出力ファイル
    gpout("{0}{1:.4f}".format("r = ", r,), "Xn", "X(n+3)", name1, name2)
